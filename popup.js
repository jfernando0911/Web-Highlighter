...
// Disable Button 
const buttonText = document.querySelector(".switchTestText");
var disableButton = document.querySelector("#checkboxTest");



chrome.storage.sync.get(["disableButtonValue"], function(result){
    console.log("Value", result.disableButtonValue)
    if(result.disableButtonValue === undefined){
        chrome.storage.sync.set({disableButtonValue: true})
        disableButton.checked = true;
        buttonText.textContent = "Disable"
        
    }else{
        // console.log(".>", result.disableButtonValue)
        disableButton.checked = result.disableButtonValue;
        result.disableButtonValue ?  buttonText.textContent = "Disable" : buttonText.textContent = "Enable";
...
disableButton.addEventListener("click", function () {
    ...
    this.checked ? buttonText.textContent = "Disable": buttonText.textContent = "Enable"
    chrome.storage.sync.set({ disableButtonValue: this.checked });
})
...
chrome.storage.onChanged.addListener(function (changes, namespace) {
    ...
    // console.log(changes.disableButtonValue !== changes.testNumber2)

    if (changes.testNumber2) {
