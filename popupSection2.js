                ...
                for (let i = 0; i <= Object.keys(tr).length; i++) {
                    ...
                    if (i === 0) {
                        ...
                        createTd.addEventListener("click", (e) => {
                            ...
                            if (e) {
                                e.target.parentNode.parentNode.remove();   //It gets removed from the UI
                                chrome.storage.local.get({ testNumber2: [] }, (items) => {
                                    var tdId = Number(e.target.parentNode.parentNode.children[0].textContent);
                                    console.log(`tdId ${tdId}`)
                                    items.testNumber2.splice(tdId, 1);      //It gets removed from de array
                                    chrome.storage.local.set(items);       //the new array gets stored
